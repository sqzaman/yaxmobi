...
*/
    ...
    public function create_links( $path = '', $param = array()){
        //  Build QueryString
        if ( is_array ( $param ) ){
            $extraParam = '';
        ...
        if ($this->currentPage != 1)
            $str .= '<li style="color:red"> <a class="previous" href="'. $path . '/?page=' . ($this->currentPage - 1) . $extraParam .'">Previous</a><li>';
        ...
        for ( $i = 1; $i < $this->numPages; $i++ ){
            ...
            if( $i == 1 || $i == $this->numPages || in_array($i, $this->range ) ){
                if ( $i == $this->currentPage ){
                    $str .= '<li><a class="active" href="'. $path . '/?page=' . $i . $extraParam .'">' . $i. '</a></li>';
                }else{
                    $str .= '<li><a href="'. $path . '/?page=' . $i . $extraParam .'">'. $i .'</a></li>';
                }
            }

            if ( $this->range[ $this->midRange -1 ] < $this->numPages -1 and $i == $this->range[$this->midRange -1 ] ){
                $str .= '...';
            }
        }

        if ( $this->currentPage < $this->numPages)
        $str .= '<li> <a class="next" href="'. $path . '/?page=' . ($this->currentPage + 1) . $extraParam .'">Next</a>';
        $str .= '</ul></div>';
        return $str;
    }
