...
<table border="0" cellspacing="0" cellpadding="0" class="container">
    ...
    <tr>
                                    ...
                                    <td valign='top' width=200>
                                        ...
                                        if ($songRating[$i]['points'][0] != 0) {
                                            ...
                                            <a href="javascript:void"
                                               onclick="openWindow('<?= MY_SITE_URL ?>ratings/add/<?= $ringtones[$i]['id'] ?>,3','');">Rate
                                            </a>
                                    </td></tr></table>
        ...
        ?> <br>
                                <a href="javascript:void"
                                   onclick="openWindow('<?= MY_SITE_URL ?>ratings/add/<?= $ringtones[$i]['id'] ?>,3','');">Be
    ...
    if ( ( $ringtones[$i]['type'] == '0' || $ringtones[$i]['type'] == '' ) || $loggedIn ) {
        ?> <script language="JavaScript"
                                src="/tone/player/audio-player.js"></script> <object
                                    type="application/x-shockwave-flash"
    ...
    ?>
                            ...
                            if (is_file($file)) {
                                ...
                                if ($ringtones[$i]['type'] == '0' || $ringtones[$i]['type'] == '') {
                                    ?> <a href="javascript: void();"
                                       onclick="javascript: document.frm1.type.value='<?= $ringtones[$i]['type'] ?>';document.frm1.price.value='<?= $ringtones[$i]['price'] ?>';document.frm1.ringtone_file.value= '<?= $fileName ?>'; document.frm1.myxer_tag.value= '<?= $ringtones[$i]['myxer_tag'] ?>';document.frm1.ringtone_title.value= '<?= $fileTitle ?>';  document.frm1.submit();">
                                    <img src='/img/images/<?php echo $downloadImage;?>' border='0'/>
                                    </a> <?
                    } else {
                                    ?> <a href="javascript: void();"
                                       onclick="javascript: if ( confirm('Your credits will be adjusted once you confirm to download this ringtone.')) { document.frm1.idf.value='<?= $ringtones[$i]['id'] ?>';document.frm1.type.value='<?= $ringtones[$i]['type'] ?>';document.frm1.price.value='<?= $ringtones[$i]['price'] ?>';document.frm1.ringtone_file.value= '<?= $fileName ?>'; document.frm1.ringtone_title.value= '<?= $fileTitle ?>';  document.frm1.submit();}">
